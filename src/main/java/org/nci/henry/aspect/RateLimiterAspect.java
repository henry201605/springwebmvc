...
import org.nci.henry.annotation.RateLimiterAnno;
...
//    @Pointcut("execution(public * org.nci.henry.controller.*.*(..))")
    @Pointcut("@annotation(org.nci.henry.annotation.RateLimiterAnno)")
    ...
    public Object around(ProceedingJoinPoint joinPoint,RateLimiterAnno rateLimiterAnno) throws Throwable {
        double v = rateLimiterAnno.permitsPerSecond();
        String msg = rateLimiterAnno.msg();
        ...
        // 获取自定义注解
        RateLimiterAnno rateLimiter = getRateLimiterAnno(joinPoint);
        if (rateLimiter != null) {
            ...
            log.info("<<=================  请求{},获取令牌桶,容量{} 成功!!!", url, rateLimiter.permitsPerSecond());
     ...
     */
    private RateLimiterAnno getRateLimiterAnno(final JoinPoint joinPoint) {
        Method[] methods = joinPoint.getTarget().getClass().getDeclaredMethods();
        String name = joinPoint.getSignature().getName();
        if (!StringUtils.isEmpty(name)) {
            for (Method method : methods) {
                RateLimiterAnno annotation = method.getAnnotation(RateLimiterAnno.class);
